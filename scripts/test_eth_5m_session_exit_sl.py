...
DEFAULT_LOG_DIR = Path(__file__).resolve().parents[1] / "logs-eth"
...
# Entry freshness gate: ETH must keep moving in the trade direction over the recent window,
# filtering stale spikes that already stalled (prone to the reversals seen in logs).
DEFAULT_ETH_VELOCITY_WINDOW_SEC = 30
DEFAULT_ETH_VELOCITY_MIN_USD = 0.40
...
def _binance_price(ts_ms: Optional[int] = None) -> Optional[float]:
    """Binance ETHUSDT price at ts_ms (kline open), or spot now if ts_ms is None."""
    try:
        if ts_ms is None:
            r = requests.get(
...
def eth_move_usd(bucket_ts: int) -> Optional[float]:
...
def eth_velocity_usd(window_sec: int) -> Optional[float]:
    # ponytail: short-window momentum. ETH change over the last `window_sec` seconds.
...
def eth_thesis_allows_exit(opened: dict[str, Any], threshold: float) -> bool:
    """
    Gate exits on the ETH move since bucket start (matches the 'move='
    field in the entry log, computed by eth_move_usd(bucket_ts)).
    ...
    Returns True (allow exit) if ETH data is missing — never block on
    ...
    if not side or not market_slug:
        return True
    # bucket_ts is the trailing number in "eth-updown-5m-<bucket_ts>".
    try:
        bucket_ts = int(market_slug.rsplit("-", 1)[-1])
    except (ValueError, IndexError):
        return True
    current_move = eth_move_usd(bucket_ts)
...
# Module-level cache for the monitor-line eth_move display. Refreshes every
# 5 seconds to keep Binance REST load low at 0.1s poll cadence.
_eth_move_display_cache = {"bucket_ts": None, "move": None, "fetched_at": 0.0}
_ETH_MOVE_DISPLAY_TTL_SEC = 5.0


def _get_eth_move_for_display(bucket_ts: int) -> Optional[float]:
    """Cached eth_move_usd for the monitor-line display. 5-second TTL."""
    now = time.time()
    if (_eth_move_display_cache["bucket_ts"] == bucket_ts
            and now - _eth_move_display_cache["fetched_at"] < _ETH_MOVE_DISPLAY_TTL_SEC):
        return _eth_move_display_cache["move"]
    move = eth_move_usd(bucket_ts)
    _eth_move_display_cache["bucket_ts"] = bucket_ts
    _eth_move_display_cache["move"] = move
    _eth_move_display_cache["fetched_at"] = now
    return move
...
def eth_move_min_for_threshold(threshold: float) -> float:
    # ponytail: ETH moves less than BTC per 5m bucket — roughly half the absolute move.
    ...
    return table.get(round(float(threshold), 2), DEFAULT_ETH_MOVE_USD_MIN)


def eth_move_min_for_price(price: float) -> float:
    for threshold in (0.95, 0.90, 0.85, 0.80, 0.75, 0.70, 0.65, 0.60):
        if price >= threshold:
            return eth_move_min_for_threshold(threshold)
    return DEFAULT_ETH_MOVE_USD_MIN
...
PROFILES: dict[str, dict[str, Any]] = {
    "conservative": {
        ...
        "eth_velocity_window_sec": DEFAULT_ETH_VELOCITY_WINDOW_SEC,
        "eth_velocity_min_usd": DEFAULT_ETH_VELOCITY_MIN_USD,
...
def apply_profile(args: argparse.Namespace) -> argparse.Namespace:
    ...
    if args.eth_move_usd_min is None:
        args.eth_move_usd_min = eth_move_min_for_threshold(args.threshold)
    return args
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--eth-move-usd-min", type=float, default=None)
    ...
    ap.add_argument("--eth-velocity-window-sec", type=int, default=None,
                    help="freshness-gate window for ETH momentum")
    ap.add_argument("--eth-velocity-min-usd", type=float, default=None,
                    help="min ETH move required over the velocity window to enter")
    ...
    ap.add_argument("--redeem-slug", default=None,
                    help="redeem a resolved market by event slug, e.g. eth-updown-5m-1781958300")
    ap.add_argument("--redeem-condition-id", default=None,
                    help="redeem a resolved market directly by conditionId (0x...)")
    args = ap.parse_args()
    args.eth_move_usd_min_user_set = args.eth_move_usd_min is not None
...
def run_cycle(
    ...
    report: dict[str, Any] = {
        ...
        "params": {
            ...
            "eth_move_usd_min": args.eth_move_usd_min,
    ...
    while time.time() < deadline:
        try:
            ...
            if not market:
                ...
                log(f"no current active ETH 5m market")
                if args.once:
            ...
            move_usd = eth_move_usd(market_bucket_ts)
            required_move_usd = (
                args.eth_move_usd_min
                if getattr(args, "eth_move_usd_min_user_set", False)
                else eth_move_min_for_price(ask_price)
            )
            report["last_eth_move_usd"] = move_usd
            report["last_required_eth_move_usd"] = required_move_usd
            if move_usd is None or (side == "UP" and move_usd < required_move_usd) or (
                side == "DOWN" and move_usd > -required_move_usd
            ):
                signal_side = None
                signal_polls = 0
                log(f"skip: {seconds_left:.0f}s left ETH move mismatch side={side} move={move_usd} min={required_move_usd}")
                if args.once:
                    report["result"] = "eth_move_mismatch"
                    break
                time.sleep(args.poll_sec)
                continue

            # Freshness gate: ETH must still be moving NOW, not just at bucket start
            if args.eth_velocity_min_usd and args.eth_velocity_min_usd > 0:
                vel = eth_velocity_usd(args.eth_velocity_window_sec)
                vel_ok = vel is None or (
                    (side == "UP" and vel >= args.eth_velocity_min_usd) or
                    (side == "DOWN" and vel <= -args.eth_velocity_min_usd)
                )
                if not vel_ok:
                    signal_side = None
                    signal_polls = 0
                    log(f"skip: {seconds_left:.0f}s left ETH velocity stale side={side} vel={vel} min=±{args.eth_velocity_min_usd}")
                    if args.once:
                        report["result"] = "eth_velocity_stale"
                        break
                    time.sleep(args.poll_sec)
                    continue
            ...
            # ETH spot price at entry moment
            ...
            report["decision"] = {
                ...
                "eth_move_usd": move_usd,
                "required_eth_move_usd": required_move_usd,
            ...
            event(f"entry candidate: {side} ask={ask_price:.3f} spread={entry_spread} eth=${eth_price_at_entry} buy=${notional:.2f}")
    ...
    try:
        while True:
            ...
            eth_move_disp = _get_eth_move_for_display(bkt_disp) if bkt_disp else None
            eth_move_str = f"{eth_move_disp:+.2f}" if eth_move_disp is not None else "n/a"
            event(
                ...
                f"{seconds_left:.0f}s left eth_move={eth_move_str}"
            )
            ...
            if sell_now_value is not None:
                ...
                if trail_activated:
                    ...
                    if sell_now_value <= trail_floor:
                        ...
                        if fresh_sell_now is not None and fresh_sell_now < trail_floor * 0.90:
                            ...
                            time.sleep(args.poll_sec)
                            continue
                        # ETH thesis gate: only fire TP if ETH move is < $10 in
                        # trade direction. Otherwise keep monitoring — all other gates remain active.
                        if not eth_thesis_allows_exit(opened, 0.5):
                            bkt = int(opened.get("market_slug", "0").rsplit("-", 1)[-1] or 0)
                            cm = eth_move_usd(bkt) if bkt else None
                            cm_str = f"${cm:+.2f}" if cm is not None else "n/a"
                            log(f"trailing_tp blocked: ETH move still in trade direction (current={cm_str}, threshold=$0.5); continuing to monitor")
                            time.sleep(args.poll_sec)
                            continue
                ...
                if sell_now_value <= stop_loss_value and seconds_left > 5:
                    ...
                    if fresh_bid is not None and fresh_bid < args.stop_loss_bid:
                        ...
                        time.sleep(args.poll_sec)
                        continue
                    # ETH thesis gate on SL: block if ETH still moving in trade direction
                    if not eth_thesis_allows_exit(opened, 0.25):
                        bkt = int(opened.get("market_slug", "0").rsplit("-", 1)[-1] or 0)
                        cm = eth_move_usd(bkt) if bkt else None
                        cm_str = f"${cm:+.2f}" if cm is not None else "n/a"
                        log(f"stop_loss blocked: ETH move still in trade direction (current={cm_str}, threshold=$0.25); continuing to monitor")
                        time.sleep(args.poll_sec)
                        continue
            ...
            if seconds_left <= 15 and not ride_settlement:
                if (args.max_bid_exit and sell_now_value is not None and current_bid is not None
                        ...
                        and sell_now_value > opened["estimated_notional_usd"]):
                    # ETH thesis gate: only fire 15s force-exit if ETH move is
                    # < $10 in trade direction or opposite. Otherwise ride.
                    if not eth_thesis_allows_exit(opened, 0.5):
                        bkt = int(opened.get("market_slug", "0").rsplit("-", 1)[-1] or 0)
                        cm = eth_move_usd(bkt) if bkt else None
                        cm_str = f"${cm:+.2f}" if cm is not None else "n/a"
                        log(f"15s force-exit blocked: ETH move still in trade direction (current={cm_str}, threshold=$0.5); continuing to monitor")
                        time.sleep(args.poll_sec)
                        continue
                    close_reason = f"force_exit_near_max_bid_{current_bid:.3f}_15s_left"
                    break
                if seconds_left <= 5:
                    # ETH thesis gate: only force-exit-in-loss if ETH move is
                    # < $5 in trade direction or opposite. Otherwise ride.
                    if not eth_thesis_allows_exit(opened, 0.25):
                        bkt = int(opened.get("market_slug", "0").rsplit("-", 1)[-1] or 0)
                        cm = eth_move_usd(bkt) if bkt else None
                        cm_str = f"${cm:+.2f}" if cm is not None else "n/a"
                        event(f"force_exit_in_loss_5s blocked: ETH thesis alive at 5s (current={cm_str}, threshold=$0.25); riding settlement")
...
def cmd_redeem(args: argparse.Namespace) -> int:
    ...
    if not condition_id:
        ...
        if not slug:
            event("error: provide --redeem-slug (e.g. eth-updown-5m-1781958300) or --redeem-condition-id")
            return 1
...
def main() -> int:
    ...
    LOG_FILE = Path(args.log_file) if args.log_file else DEFAULT_LOG_DIR / f"eth_5m_{now_utc():%Y%m%d_%H%M%S}.log"
    log(
        "started "
