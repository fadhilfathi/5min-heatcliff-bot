...
CTF_ABI = [
    ...
    {"name": "setApprovalForAll", "type": "function", "stateMutability": "nonsolable",
     "inputs": [{"name": "operator", "type": "address"}, {"name": "approved", "type": "bool"}],
     "outputs": []},
]
ADAPTER_ABI = [
    {"name": "redeemPositions", "type": "function", "stateMutability": "nonsolable",
     "inputs": [
...
DEFAULT_LOG_DIR = Path(__file__).resolve().parents[1] / "logs-sol"
...
# Entry freshness gate: sol must keep moving in the trade direction over the recent window,
# filtering stale spikes that already stalled (prone to the reversals seen in logs).
DEFAULT_sol_VELOCITY_WINDOW_SEC = 30
DEFAULT_sol_VELOCITY_MIN_USD = 0.04
...
def _binance_price(ts_ms: Optional[int] = None) -> Optional[float]:
    """Binance SOLUSDT price at ts_ms (kline open), or spot now if ts_ms is None."""
    try:
        if ts_ms is None:
            r = requests.get(
...
def sol_move_usd(bucket_ts: int) -> Optional[float]:
...
def sol_velocity_usd(window_sec: int) -> Optional[float]:
    # ponytail: short-window momentum. sol change over the last `window_sec` seconds.
...
def sol_thesis_allows_exit(opened: dict[str, Any], threshold: float) -> bool:
    """
    Gate exits on the sol move since bucket start (matches the 'move='
    field in the entry log, computed by sol_move_usd(bucket_ts)).
    ...
    Returns True (allow exit) if sol data is missing — never block on
    ...
    if not side or not market_slug:
        return True
    # bucket_ts is the trailing number in "sol-updown-5m-<bucket_ts>".
    try:
        bucket_ts = int(market_slug.rsplit("-", 1)[-1])
    except (ValueError, IndexError):
        return True
    current_move = sol_move_usd(bucket_ts)
...
# Module-level cache for the monitor-line sol_move display. Refreshes every
# 5 seconds to keep Binance REST load low at 0.1s poll cadence.
_sol_move_display_cache = {"bucket_ts": None, "move": None, "fetched_at": 0.0}
_sol_MOVE_DISPLAY_TTL_SEC = 5.0


def _get_sol_move_for_display(bucket_ts: int) -> Optional[float]:
    """Cached sol_move_usd for the monitor-line display. 5-second TTL."""
    now = time.time()
    if (_sol_move_display_cache["bucket_ts"] == bucket_ts
            and now - _sol_move_display_cache["fetched_at"] < _sol_MOVE_DISPLAY_TTL_SEC):
        return _sol_move_display_cache["move"]
    move = sol_move_usd(bucket_ts)
    _sol_move_display_cache["bucket_ts"] = bucket_ts
    _sol_move_display_cache["move"] = move
    _sol_move_display_cache["fetched_at"] = now
    return move
...
def sol_move_min_for_threshold(threshold: float) -> float:
    # ponytail: sol moves less than BTC per 5m bucket — roughly half the absolute move.
    ...
    return table.get(round(float(threshold), 2), DEFAULT_SOL_MOVE_USD_MIN)


def sol_move_min_for_price(price: float) -> float:
    for threshold in (0.95, 0.90, 0.85, 0.80, 0.75, 0.70, 0.65, 0.60):
        if price >= threshold:
            return sol_move_min_for_threshold(threshold)
    return DEFAULT_SOL_MOVE_USD_MIN
...
PROFILES: dict[str, dict[str, Any]] = {
    "conservative": {
        ...
        "sol_velocity_window_sec": DEFAULT_sol_VELOCITY_WINDOW_SEC,
        "sol_velocity_min_usd": DEFAULT_sol_VELOCITY_MIN_USD,
...
def apply_profile(args: argparse.Namespace) -> argparse.Namespace:
    ...
    if args.sol_move_usd_min is None:
        args.sol_move_usd_min = sol_move_min_for_threshold(args.threshold)
    return args
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--sol-move-usd-min", type=float, default=None)
    ...
    ap.add_argument("--sol-velocity-window-sec", type=int, default=None,
                    help="freshness-gate window for sol momentum")
    ap.add_argument("--sol-velocity-min-usd", type=float, default=None,
                    help="min sol move required over the velocity window to enter")
    ...
    ap.add_argument("--redeem-slug", default=None,
                    help="redeem a resolved market by event slug, e.g. sol-updown-5m-1781958300")
    ap.add_argument("--redeem-condition-id", default=None,
                    help="redeem a resolved market directly by conditionId (0x...)")
    args = ap.parse_args()
    args.sol_move_usd_min_user_set = args.sol_move_usd_min is not None
...
def run_cycle(
    ...
    report: dict[str, Any] = {
        ...
        "params": {
            ...
            "sol_move_usd_min": args.sol_move_usd_min,
    ...
    while time.time() < deadline:
        try:
            ...
            if not market:
                ...
                log(f"no current active sol 5m market")
                if args.once:
            ...
            move_usd = sol_move_usd(market_bucket_ts)
            required_move_usd = (
                args.sol_move_usd_min
                if getattr(args, "sol_move_usd_min_user_set", False)
                else sol_move_min_for_price(ask_price)
            )
            report["last_sol_move_usd"] = move_usd
            report["last_required_sol_move_usd"] = required_move_usd
            if move_usd is None or (side == "UP" and move_usd < required_move_usd) or (
                side == "DOWN" and move_usd > -required_move_usd
            ):
                signal_side = None
                signal_polls = 0
                log(f"skip: {seconds_left:.0f}s left sol move mismatch side={side} move={move_usd} min={required_move_usd}")
                if args.once:
                    report["result"] = "sol_move_mismatch"
                    break
                time.sleep(args.poll_sec)
                continue

            # Freshness gate: sol must still be moving NOW, not just at bucket start
            if args.sol_velocity_min_usd and args.sol_velocity_min_usd > 0:
                vel = sol_velocity_usd(args.sol_velocity_window_sec)
                vel_ok = vel is None or (
                    (side == "UP" and vel >= args.sol_velocity_min_usd) or
                    (side == "DOWN" and vel <= -args.sol_velocity_min_usd)
                )
                if not vel_ok:
                    signal_side = None
                    signal_polls = 0
                    log(f"skip: {seconds_left:.0f}s left sol velocity stale side={side} vel={vel} min=±{args.sol_velocity_min_usd}")
                    if args.once:
                        report["result"] = "sol_velocity_stale"
                        break
                    time.sleep(args.poll_sec)
                    continue
            ...
            # sol spot price at entry moment
            sol_price_at_entry = _binance_price()
            report["decision"] = {
                "ts": ts_utc(),
                "market_slug": slug,
                "side": side,
            ...
            event(f"entry candidate: {side} ask={ask_price:.3f} spread={entry_spread} sol=${sol_price_at_entry} buy=${notional:.2f}")
    ...
    try:
        while True:
            ...
            sol_move_disp = _get_sol_move_for_display(bkt_disp) if bkt_disp else None
            sol_move_str = f"{sol_move_disp:+.2f}" if sol_move_disp is not None else "n/a"
            event(
                ...
                f"{seconds_left:.0f}s left sol_move={sol_move_str}"
            )
            ...
            if sell_now_value is not None:
                ...
                if trail_activated:
                    ...
                    if sell_now_value <= trail_floor:
                        ...
                        if fresh_sell_now is not None and fresh_sell_now < trail_floor * 0.90:
                            ...
                            time.sleep(args.poll_sec)
                            continue
                        # sol thesis gate: only fire TP if sol move is < $10 in
                        # trade direction. Otherwise keep monitoring — all other gates remain active.
                        if not sol_thesis_allows_exit(opened, 0.05):
                            bkt = int(opened.get("market_slug", "0").rsplit("-", 1)[-1] or 0)
                            cm = sol_move_usd(bkt) if bkt else None
                            cm_str = f"${cm:+.2f}" if cm is not None else "n/a"
                            log(f"trailing_tp blocked: SOL move still in trade direction (current={cm_str}, threshold=$0.05); continuing to monitor")
                            time.sleep(args.poll_sec)
                            continue
                ...
                if sell_now_value <= stop_loss_value and seconds_left > 5:
                    ...
                    if fresh_bid is not None and fresh_bid < args.stop_loss_bid:
                        ...
                        time.sleep(args.poll_sec)
                        continue
                    # SOL thesis gate on SL: block if SOL still moving in trade direction
                    if not sol_thesis_allows_exit(opened, 0.02):
                        bkt = int(opened.get("market_slug", "0").rsplit("-", 1)[-1] or 0)
                        cm = sol_move_usd(bkt) if bkt else None
                        cm_str = f"${cm:+.2f}" if cm is not None else "n/a"
                        log(f"stop_loss blocked: SOL move still in trade direction (current={cm_str}, threshold=$0.02); continuing to monitor")
                        time.sleep(args.poll_sec)
                        continue
            ...
            if seconds_left <= 15 and not ride_settlement:
                if (args.max_bid_exit and sell_now_value is not None and current_bid is not None
                        ...
                        and sell_now_value > opened["estimated_notional_usd"]):
                    # sol thesis gate: only fire 15s force-exit if sol move is
                    # < $10 in trade direction or opposite. Otherwise ride.
                    if not sol_thesis_allows_exit(opened, 0.05):
                        bkt = int(opened.get("market_slug", "0").rsplit("-", 1)[-1] or 0)
                        cm = sol_move_usd(bkt) if bkt else None
                        cm_str = f"${cm:+.2f}" if cm is not None else "n/a"
                        log(f"15s force-exit blocked: SOL move still in trade direction (current={cm_str}, threshold=$0.05); continuing to monitor")
                        time.sleep(args.poll_sec)
                        continue
                    close_reason = f"force_exit_near_max_bid_{current_bid:.3f}_15s_left"
                    break
                if seconds_left <= 5:
                    # sol thesis gate: only force-exit-in-loss if sol move is
                    # < $5 in trade direction or opposite. Otherwise ride.
                    if not sol_thesis_allows_exit(opened, 0.02):
                        bkt = int(opened.get("market_slug", "0").rsplit("-", 1)[-1] or 0)
                        cm = sol_move_usd(bkt) if bkt else None
                        cm_str = f"${cm:+.2f}" if cm is not None else "n/a"
                        event(f"force_exit_in_loss_5s blocked: SOL thesis alive at 5s (current={cm_str}, threshold=$0.02); riding settlement")
    ...
    try:
        ...
        if pre_close_bid is not None and pre_close_bid < args.stop_loss_bid:
            ...
            return report
        client = auth_client()
        close_post, ladder_msolod, amount_shares = close_with_ladder(client, opened, args.close_retry_max)
        ...
        sol_price_at_close = _binance_price()
        sol_drift = (
            round(sol_price_at_close - opened["sol_price_at_entry"], 2)
            if sol_price_at_close is not None and opened.get("sol_price_at_entry") is not None
            else None
        )
        report["closed"] = {
            "closed_at": ts_utc(),
            ...
            "close_msolod": ladder_msolod,
            "peak_sell_now": peak_sell_now,
            "sol_price_at_close": sol_price_at_close,
            "sol_drift_usd": sol_drift,
        }
        report["result"] = "done"
        ...
        tape("")
        event(f"close submitted via {ladder_msolod}")
    except Exception as exc:
...
def redeem_position(condition_id: str, rpc_url: str, private_key: str) -> str:
    ...
    ctf = w3.sol.contract(address=Web3.to_checksum_address(CTF_ADDRESS), abi=CTF_ABI)
    ...
    if not ctf.functions.isApprovedForAll(account.address, adapter_addr).call():
        ...
        approve_tx = ctf.functions.setApprovalForAll(adapter_addr, True).build_transaction({
            ...
            "nonce": w3.sol.get_transaction_count(account.address),
            "gas": 100_000,
            "gasPrice": w3.sol.gas_price,
        })
        signed = account.sign_transaction(approve_tx)
        receipt = w3.sol.wait_for_transaction_receipt(w3.sol.send_raw_transaction(signed.raw_transaction))
    ...
    adapter = w3.sol.contract(address=adapter_addr, abi=ADAPTER_ABI)
...
def cmd_redeem(args: argparse.Namespace) -> int:
    ...
    if not condition_id:
        ...
        if not slug:
            event("error: provide --redeem-slug (e.g. sol-updown-5m-1781958300) or --redeem-condition-id")
            return 1
...
def main() -> int:
    ...
    LOG_FILE = Path(args.log_file) if args.log_file else DEFAULT_LOG_DIR / f"sol_5m_{now_utc():%Y%m%d_%H%M%S}.log"
    log(
        "started "
