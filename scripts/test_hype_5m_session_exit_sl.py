...
DEFAULT_LOG_DIR = Path(__file__).resolve().parents[1] / "logs-hype"
...
DEFAULT_HYPE_MOVE_USD_MIN = 0.03
...
# Entry freshness gate: HYPE must keep moving in the trade direction over the recent window,
# filtering stale spikes that already stalled (prone to the reversals seen in logs).
DEFAULT_HYPE_VELOCITY_WINDOW_SEC = 30
DEFAULT_HYPE_VELOCITY_MIN_USD = 0.006
...
def _binance_price(ts_ms: Optional[int] = None) -> Optional[float]:
    """Bybit HYPEUSDT price at ts_ms (1m kline open), or spot now if ts_ms is None."""
    try:
        if ts_ms is None:
            r = requests.get(
                "https://api.bybit.com/v5/market/tickers",
                params={"category": "spot", "symbol": BINANCE_SYMBOL},
                timeout=5,
            )
        ...
        r = requests.get(
            ...
            params={"category": "spot", "symbol": BINANCE_SYMBOL, "interval": "1",
                    "start": ts_ms, "end": ts_ms + 59999, "limit": 1},
            timeout=5,
        )
        items = r.json().get("result", {}).get("list", [])
        return float(items[0][1]) if items else None  # index 1 = open price
    except Exception:
        return None


def hype_move_usd(bucket_ts: int) -> Optional[float]:
...
def hype_velocity_usd(window_sec: int) -> Optional[float]:
    # ponytail: short-window momentum. HYPE change over the last `window_sec` seconds.
...
def hype_thesis_allows_exit(opened: dict[str, Any], threshold: float) -> bool:
    """
    Gate exits on the HYPE move since bucket start (matches the 'move='
    field in the entry log, computed by hype_move_usd(bucket_ts)).
    ...
    Returns True (allow exit) if HYPE data is missing — never block on
    ...
    if not side or not market_slug:
        return True
    # bucket_ts is the trailing number in "hype-updown-5m-<bucket_ts>".
    try:
        bucket_ts = int(market_slug.rsplit("-", 1)[-1])
    except (ValueError, IndexError):
        return True
    current_move = hype_move_usd(bucket_ts)
...
# Module-level cache for the monitor-line hype_move display. Refreshes every
# 5 seconds to keep Binance REST load low at 0.1s poll cadence.
_hype_move_display_cache = {"bucket_ts": None, "move": None, "fetched_at": 0.0}
_HYPE_MOVE_DISPLAY_TTL_SEC = 5.0


def _get_hype_move_for_display(bucket_ts: int) -> Optional[float]:
    """Cached hype_move_usd for the monitor-line display. 5-second TTL."""
    now = time.time()
    if (_hype_move_display_cache["bucket_ts"] == bucket_ts
            and now - _hype_move_display_cache["fetched_at"] < _HYPE_MOVE_DISPLAY_TTL_SEC):
        return _hype_move_display_cache["move"]
    move = hype_move_usd(bucket_ts)
    _hype_move_display_cache["bucket_ts"] = bucket_ts
    _hype_move_display_cache["move"] = move
    _hype_move_display_cache["fetched_at"] = now
    return move
...
def hype_move_min_for_threshold(threshold: float) -> float:
    # ponytail: HYPE moves ~11.13% of BNB per 5m bucket (ratio 62.05/557.5).
    ...
    return table.get(round(float(threshold), 2), DEFAULT_HYPE_MOVE_USD_MIN)


def hype_move_min_for_price(price: float) -> float:
    for threshold in (0.95, 0.90, 0.85, 0.80, 0.75, 0.70, 0.65, 0.60):
        if price >= threshold:
            return hype_move_min_for_threshold(threshold)
    return DEFAULT_HYPE_MOVE_USD_MIN
...
PROFILES: dict[str, dict[str, Any]] = {
    "conservative": {
        ...
        "hype_velocity_window_sec": DEFAULT_HYPE_VELOCITY_WINDOW_SEC,
        "hype_velocity_min_usd": DEFAULT_HYPE_VELOCITY_MIN_USD,
...
def apply_profile(args: argparse.Namespace) -> argparse.Namespace:
    ...
    if args.hype_move_usd_min is None:
        args.hype_move_usd_min = hype_move_min_for_threshold(args.threshold)
    return args
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--hype-move-usd-min", type=float, default=None)
    ...
    ap.add_argument("--hype-velocity-window-sec", type=int, default=None,
                    help="freshness-gate window for HYPE momentum")
    ap.add_argument("--hype-velocity-min-usd", type=float, default=None,
                    help="min HYPE move required over the velocity window to enter")
    ...
    ap.add_argument("--redeem-slug", default=None,
                    help="redeem a resolved market by event slug, e.g. hype-updown-5m-1781958300")
    ap.add_argument("--redeem-condition-id", default=None,
                    help="redeem a resolved market directly by conditionId (0x...)")
    args = ap.parse_args()
    args.hype_move_usd_min_user_set = args.hype_move_usd_min is not None
...
def run_cycle(
    ...
    report: dict[str, Any] = {
        ...
        "params": {
            ...
            "hype_move_usd_min": args.hype_move_usd_min,
    ...
    while time.time() < deadline:
        try:
            ...
            if not market:
                ...
                log(f"no current active HYPE 5m market")
                if args.once:
            ...
            move_usd = hype_move_usd(market_bucket_ts)
            required_move_usd = (
                args.hype_move_usd_min
                if getattr(args, "hype_move_usd_min_user_set", False)
                else hype_move_min_for_price(ask_price)
            )
            report["last_hype_move_usd"] = move_usd
            report["last_required_hype_move_usd"] = required_move_usd
            if move_usd is None or (side == "UP" and move_usd < required_move_usd) or (
                side == "DOWN" and move_usd > -required_move_usd
            ):
                signal_side = None
                signal_polls = 0
                log(f"skip: {seconds_left:.0f}s left HYPE move mismatch side={side} move={move_usd} min={required_move_usd}")
                if args.once:
                    report["result"] = "hype_move_mismatch"
                    break
                time.sleep(args.poll_sec)
                continue

            # Freshness gate: HYPE must still be moving NOW, not just at bucket start
            if args.hype_velocity_min_usd and args.hype_velocity_min_usd > 0:
                vel = hype_velocity_usd(args.hype_velocity_window_sec)
                vel_ok = vel is None or (
                    (side == "UP" and vel >= args.hype_velocity_min_usd) or
                    (side == "DOWN" and vel <= -args.hype_velocity_min_usd)
                )
                if not vel_ok:
                    signal_side = None
                    signal_polls = 0
                    log(f"skip: {seconds_left:.0f}s left HYPE velocity stale side={side} vel={vel} min=±{args.hype_velocity_min_usd}")
                    if args.once:
                        report["result"] = "hype_velocity_stale"
                        break
                    time.sleep(args.poll_sec)
                    continue
            ...
            # HYPE spot price at entry moment
            hype_price_at_entry = _binance_price()
            report["decision"] = {
                "ts": ts_utc(),
                "market_slug": slug,
                "side": side,
            ...
            event(f"entry candidate: {side} ask={ask_price:.3f} spread={entry_spread} hype=${hype_price_at_entry} buy=${notional:.2f}")
    ...
    try:
        while True:
            ...
            hype_move_disp = _get_hype_move_for_display(bkt_disp) if bkt_disp else None
            hype_move_str = f"{hype_move_disp:+.2f}" if hype_move_disp is not None else "n/a"
            event(
                ...
                f"{seconds_left:.0f}s left hype_move={hype_move_str}"
            )
            ...
            if sell_now_value is not None:
                ...
                if trail_activated:
                    ...
                    if sell_now_value <= trail_floor:
                        ...
                        if fresh_sell_now is not None and fresh_sell_now < trail_floor * 0.90:
                            ...
                            time.sleep(args.poll_sec)
                            continue
                        # HYPE thesis gate: only fire TP if HYPE move is < $0.01 in
                        # trade direction. Otherwise keep monitoring — all other gates remain active.
                        if not hype_thesis_allows_exit(opened, 0.005):
                            bkt = int(opened.get("market_slug", "0").rsplit("-", 1)[-1] or 0)
                            cm = hype_move_usd(bkt) if bkt else None
                            cm_str = f"${cm:+.2f}" if cm is not None else "n/a"
                            log(f"trailing_tp blocked: HYPE move still in trade direction (current={cm_str}, threshold=$0.005); continuing to monitor")
                            time.sleep(args.poll_sec)
                            continue
                ...
                if sell_now_value <= stop_loss_value and seconds_left > 5:
                    ...
                    if fresh_bid is not None and fresh_bid < args.stop_loss_bid:
                        ...
                        time.sleep(args.poll_sec)
                        continue
                    # HYPE thesis gate on SL: block if HYPE still moving in trade direction
                    if not hype_thesis_allows_exit(opened, 0.0025):
                        bkt = int(opened.get("market_slug", "0").rsplit("-", 1)[-1] or 0)
                        cm = hype_move_usd(bkt) if bkt else None
                        cm_str = f"${cm:+.2f}" if cm is not None else "n/a"
                        log(f"stop_loss blocked: HYPE move still in trade direction (current={cm_str}, threshold=$0.0025); continuing to monitor")
                        time.sleep(args.poll_sec)
                        continue
            ...
            if seconds_left <= 15 and not ride_settlement:
                if (args.max_bid_exit and sell_now_value is not None and current_bid is not None
                        ...
                        and sell_now_value > opened["estimated_notional_usd"]):
                    # HYPE thesis gate: only fire 15s force-exit if HYPE move is
                    # < $0.01 in trade direction or opposite. Otherwise ride.
                    if not hype_thesis_allows_exit(opened, 0.005):
                        bkt = int(opened.get("market_slug", "0").rsplit("-", 1)[-1] or 0)
                        cm = hype_move_usd(bkt) if bkt else None
                        cm_str = f"${cm:+.2f}" if cm is not None else "n/a"
                        log(f"15s force-exit blocked: HYPE move still in trade direction (current={cm_str}, threshold=$0.005); continuing to monitor")
                        time.sleep(args.poll_sec)
                        continue
                    close_reason = f"force_exit_near_max_bid_{current_bid:.3f}_15s_left"
                    break
                if seconds_left <= 5:
                    # HYPE thesis gate: only force-exit-in-loss if HYPE move is
                    # < $0.006 in trade direction or opposite. Otherwise ride.
                    if not hype_thesis_allows_exit(opened, 0.0025):
                        bkt = int(opened.get("market_slug", "0").rsplit("-", 1)[-1] or 0)
                        cm = hype_move_usd(bkt) if bkt else None
                        cm_str = f"${cm:+.2f}" if cm is not None else "n/a"
                        event(f"force_exit_in_loss_5s blocked: HYPE thesis alive at 5s (current={cm_str}, threshold=$0.0025); riding settlement")
    ...
    try:
        ...
        hype_price_at_close = _binance_price()
        hype_drift = (
            round(hype_price_at_close - opened["hype_price_at_entry"], 2)
            if hype_price_at_close is not None and opened.get("hype_price_at_entry") is not None
            else None
        )
        report["closed"] = {
            "closed_at": ts_utc(),
            ...
            "hype_price_at_close": hype_price_at_close,
            "hype_drift_usd": hype_drift,
        }
        report["result"] = "done"
...
def cmd_redeem(args: argparse.Namespace) -> int:
    ...
    if not condition_id:
        ...
        if not slug:
            event("error: provide --redeem-slug (e.g. hype-updown-5m-1781958300) or --redeem-condition-id")
            return 1
...
def main() -> int:
    ...
    LOG_FILE = Path(args.log_file) if args.log_file else DEFAULT_LOG_DIR / f"hype_5m_{now_utc():%Y%m%d_%H%M%S}.log"
    log(
        "started "
